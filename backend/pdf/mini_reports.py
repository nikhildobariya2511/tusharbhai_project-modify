...
async def upload_pdf(files: list[UploadFile] = File(...)):
    if not (1 <= len(files) <= 2):
        raise HTTPException(
            status_code=400,
            detail="You can upload minimum 1 and maximum 2 PDF files"
        )
    ...
    for pdf in files:
        if pdf.content_type != "application/pdf":
            raise HTTPException(
                status_code=400,
                detail=f"{pdf.filename} is not a valid PDF"
            )
...
def parse_gia_report_from_bytes(pdf_bytes: bytes, filename: str):
    ...
    barcode10_number, barcode10_path = _generate_unique_barcode(
        digits=10,
        save_path_no_ext=os.path.join(report_folder, "barcode10")
    )

    barcode12_number, barcode12_path = _generate_unique_barcode(
        digits=12,
        save_path_no_ext=os.path.join(report_folder, "barcode12")
    )
    ...
    parsed_data["Images"] = {
        ...
        "Barcode10": {
            ...
            "image": barcode10_path
        },
        "Barcode12": {
            "number": barcode12_number,
            "image": barcode12_path
        }
    }
...
def _parse_text_to_json(text: str):
    ...
    shape = search(r"Shape\s+and\s+Cutting\s+Style\s*[.:\s]*([A-Za-z\s]+?)(?=\n|$)") or search(r"([A-Za-z\s]+?Brilliant)")
    ...
    clarity_grade = search(r"Clarity\s+Grade\s*[.:\s]*([A-Z0-9]{2,3})")
    ...
    inscription = search(r"Inscription\(s\):\s*([^\n]+)")
